...
smr_run = np.pi/2
#run_folder = 'n %i c %i baR_0 %1.2f smr %1.2f'%(concentration,n,0.2,rad_to_deg(smr_run))
run_folder = 'n %i rho const baR_0 %1.2f smr %1.2f'%(n,0.2,rad_to_deg(smr_run))
offset = 0
size_per_rank = 10
    ...
    if rank == node_index:
        if batch_num == 0:
            for i in range(size_per_rank):
                get_RA_data(True,rank+i*7,outputpath,'linear',n,baR_0,smr_run,smr_run)
                print('batch1 rank'+str(rank)+': task finished '+str(rank+i*7)+'out of total'+str(14*size_per_rank-1))
        if batch_num == 1:
            for i in range(size_per_rank):
                get_RA_data(True,(rank+(i+size_per_rank-1)*7),outputpath,'log',n,baR_0,smr_run,smr_run)
